...
public struct BaseInputField: View {
    ...
    init(
        ...
        isSecure: Bool = false,
        shape: RoundedRectangle = InputFieldShape.default,
        textFont: Font = GazginTheme.typeface.body.medium,
    ...
    public init<Leading: View, Trailing: View>(
        ...
        self.init(
            ...
            shape: shape,
            textFont: textFont,
            leadingIcon: (Leading.self == EmptyView.self) ? nil : AnyView(leading),
            trailingIcon: (Trailing.self == EmptyView.self) ? nil : AnyView(trailing)
        )
    }
